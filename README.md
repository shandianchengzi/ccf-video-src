...
| CCF账号 | 首页账号卡片或「CCF账号」分类，打开官方登录页；也可导入本人的数字图书馆 Cookie |
| 研究方向 | 对标题及原站系列做可解释的多标签匹配，同时保留年份 / 系列 / 排序筛选 |
| 播放 | 高清、标清、流畅、原画；每次播放重新检查账号权限和媒体地址 |
| 自动维护 | 每天北京时间 03:23，同步公开列表和各系列归属；代码推送、手动运行也会触发 |
...
Cookie 保存在影视仓应用的设备私有存储中。「已配置」只表示有本地凭据，不表示会话仍有效。有效性及会员 / 参会 / 购买资格由 CCF 播放时确认。清除本源凭据不会清除其他视频源的 Cookie；单点登录还可能存在独立会话，可在官网退出。
...
3. 按原站总条数翻页抓取列表，再遍历原站系列获取准确归属，限速并对临时错误重试。
4. 仅将白名单元数据、dex JAR、订阅 JSON 和静态页面打包，生成 MD5 / SHA-256 校验。
5. 部署 Pages。重复页、异常空页、结构变化或目录较上次减少超过 10% 时停止，不覆盖已发布目录。
